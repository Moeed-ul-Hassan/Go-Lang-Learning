...
type User struct {
	...
	ID    primitive.ObjectID `json:"id" bson:"_id,omitempty"` //bson:"_id,omitempty"` means if the ID is empty, it will be omitted. Omit means it will not be sent to the database.
	Name  string             `json:"name" bson:"name"`
	Email string             `json:"email" bson:"email"`
}

// Configuration constants for our database connection. currently Our Database is running on localhost:27017 means Locally over only on Our System
const connectionString = "mongodb://localhost:27017"
const dbName = "go_mastery"
const colName = "users" //colName Means Collection of Names. As Defined in User Struct...

// collection is a package-level variable that holds the reference to our MongoDB collection.
// This allows us to reuse the connection throughout the application. As Defining A Global Variables
var collection *mongo.Collection

/*
init() is a special function in Go that runs automatically before main().
We use it here to set up our database connection once when the program starts. it is necessary to run the program.
*/
func init() {
...
//What I have learnt from this file is that How to create a connection between Mongo and GO
// { 
// First we Have Mongo Installed...
// Secondly we have GO Lang Installed...
// Thirdly we will add Mongo Driver installed...
// Fourthly we will create a connection between Mongo and GO Lang...
// Fifthly we will create a User Struct where we will add user info... in our userID we used primitive.objectid this means it will be randomly generated
// Sixthly we will create a function to insert the user into the database.
// Seventhly we will connect Mongo to GO by Configurations like Mongo connection string, Database name, Collection name, etc.
// Eighthly we will create an special function named "init()" that will run automatically before main() without it program will not run.
// Ninthly we will create a global variable named "collection" that will hold the reference to our MongoDB collection.
// }
//We will create a function named "insertUser()" that will insert a user into the database.
//We will create a security Guard that will close the app if app hangs, close it by Defer.Cancel()
//In main function all the other functions will be called, user will be added, other things will happens.
